...
\setlength\parindent{24pt}        The full addictive model is described by: $$Y = \beta_0 + \beta_{Major\_category} + \beta_{Total} + \beta_{Men} + \beta_{Women} + \beta_{ShareWomen} + \epsilon$$ \setlength\parindent{24pt} Running this model through the step-wise function using AIC as our criterion ended up removing too many predictors; thus, it was decided to check for interactions to see if this new model would help with this issue. Then, a step-wise process was run to reduce the model's AIC. This process aimed to remove the predictor women because the p-value was large (p = 0.7394 \> $\alpha$ = 0.05).

The final reduced model is described by: $\widehat{Median^{-1}}= (2.71 \times 10^{-5}) - (3.44 \times10^{-6}) Computers \& Mathematics - (8.87 \times 10^{-6}) Engineering - (3.99 \times 10^{-7}) Health - (3.09 \times10^{-6}) Physical Sciences - (4.14 \times 10^{-11})Men + (1.08 \times 10^{-6}) ShareWomen + (8.98 \times 10^{-11}) Men:ShareWomen$.
