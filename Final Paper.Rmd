...
\setlength\parindent{24pt}        For the purposes of our exploratory data analysis, `Major_category` was set as a factor to explore the variation of share of women within major categories and the median wages for those major categories. Within the STEM majors, median wage ranges from $\$26,000$ for Zoology and $\$110,000$ for Petroleum Engineering (Mdn = $\$44350$, M = $\$46118$). Through a stacked barplot of gender proportions per major category, we see that the biggest proportion of women chose fields related to *Health* and the biggest proportion of men chose fields related to *Engineering* (Figure 1). This is congruent with the gender roles and personality characteristics associated with women and men.\newline
...
\setlength\parindent{24pt}        We selected the column for median wage, `Median`, as our response variable. While checking normality, linearity, and constant variance, we noticed the data for `Median` showed some right skewing. Accordingly, a Box-Cox test was performed to see if a transformation was necessary (see Figure 4). The resulting rounded power was -1, suggesting that an inverse transformation of the response was required to help with right-skewedness. However, this transformation would later complicate the interpretability of the model.
...
\setlength\parindent{24pt}        To confirm that the goal of creating a predictive model for median wages was achieved, a prediction interval of (`Median`)$^{-1}$ for selected majors was run, giving the prediction interval as seen below in Table 1. \newline
...
***Table 1:***  Prediction intervals for Statistics & Decision Science, Petroleum Engineering, Zoology, Astronomy & Astrophysics and Nursing. \newline
...
Our final model has the capacity of predicting the median wage of STEM majors on the major category, total number of men in the major and total proportion of women in the major. We have tested our model in each major category, having the least success with majors that have median wages at the extremes of the dataset or are outliers within their major category (i.e. Petroleum Engineering, Zoology, and Astronomy & Astrophysics).

The most ‘important’ conclusion from our research is that since Petroleum Engineering has the highest median salary in this data set (i.e. $\$110,000$), potential students should consider majoring in this field if they only care about the median salary. But then if more women go into this field the median wage could potentially decrease.
