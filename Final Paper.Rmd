...
\setlength\parindent{24pt} The full addictive model is described by:
$$Y = \beta_0 + \beta_{Total} + \beta_{Men} + \beta_{Women} + \beta_{ShareWomen} + \beta_{Majorcategory} + \epsilon$$. 
\setlength\parindent{24pt} This step removed too many predictors; so, it was decided to check for interactions to see if this new model would help with this issue. Then a step-wise process was run to reduce the model’s AIC. This process allowed to remove the predictor women because the p-value was large (p = 0.7394 > $\alpha$ = 0.05). 
The final reduced model is described by: 
\scriptsize$$Y^{-1}= 2.71 \cdot (10)^{-5} - 3.441 \cdot (10)^{-6} x_1 - 8.87 \cdot (10)^{-6} x_2 - 3.991 \cdot (10)^{-7} x_3 - 3.09 \cdot (10)^{-6} x_4 - 4.14 \cdot (10)^{-11} x_5 + 1.08 \cdot (10)^{-6} x_6 \cdot 8.977 \cdot (10)^{-11} x_5 + x_6$$.
